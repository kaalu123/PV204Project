...
if (os.path.isfile(FILENAME) == False):
    print(
        "No source file clear.txt present to encrypt and hide using the secret.Please create clear.txt. Exiting .....")
    exit(0)
...
def RSA_users_key_cert_gen(s,
                           t):  # Usage of classess of cryptography library referenced from https://cryptography.io/en/latest/
...
def create_shares(t, s, secret):
    ...
    # print("Secret:\t", secret)
    print(
        "===Initial Shares are as below which will be encrypted and thereafter plaintext of shares will never be available after creation\n")
    time.sleep(2)
    for x in range(0, s):
...
def distribute_shares(s, shares):
    ...
    print(
        "Starting distribution of shares to users after authenticating their certificates and sending each user his share encrypted by his public key")
    time.sleep(2)
    for x in range(1, s + 1):
        try:
...
def recollect_shares(t):
    ...
    print(
        "Recollecting shares from T(Threshold) no of users for reconstructing the secret after validating their certificates")
    time.sleep(2)
    for x in range(1, t + 1):
        ...
        try:
            ...
            os.remove(dir_recollected_share)
        except:
...
def main():
    ...
    if cmd == '1':
        ...
        print(
            f" You have provided the following parameters for this multi party system:\nNo of Users:{s}\nThreshold:{t}\nSecret:{secret}\n")
        print(
            f" In our setup we will depict one CA and other {s} users : Generation of keypairs and certificates for all users might take some time. Please be patient...........\n")
        time.sleep(2)
        CA_key_cert_gen()
